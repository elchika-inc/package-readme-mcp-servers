...
export class ConanPackageReadmeMcpServer {
  ...
  private setupHandlers(): void {
    ...
    (this.server as any).setRequestHandler(CallToolRequestSchema, async (request: any, _extra: any) => {
      ...
      try {
        ...
        if (error instanceof ConanPackageReadmeMcpError) {
          throw new McpError(
  ...
  private validateGetPackageInfoParams(args: unknown): GetPackageInfoParams {
    ...
    if (params.include_options !== undefined && typeof params.include_options !== 'boolean') {
      throw new McpError(
        ErrorCode.InvalidParams,
        'include_options must be a boolean'
      );
    }
    ...
    if (params.include_options !== undefined) {
      result.include_options = params.include_options as boolean;
    }
